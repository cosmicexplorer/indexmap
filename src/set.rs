...
impl<T, S, A> Entries for IndexSet<T, S, A>
where
    A: Allocator,
{
    type Entry = Bucket<T>;
...
impl<T, S, A> fmt::Debug for IndexSet<T, S, A>
where
    T: fmt::Debug,
    A: Allocator,
{
    ...
    A: Allocator,
{
    ...
    where
        A: Clone + Default,
    {
        Self::with_capacity_and_hasher_in(n, hash_builder, A::default())
    }
...
impl<T, S, A> FromIterator<T> for IndexSet<T, S, A>
where
    T: Hash + Eq,
...
impl<T, S, A> Default for IndexSet<T, S, A>
where
    S: Default,
    A: Allocator + Clone + Default,
{
...
where
    ...
    S2: BuildHasher,
    A1: Allocator + Clone + Default,
    A2: Allocator,
{
    type Output = IndexSet<T, S1, A1>;
...
impl<T, S1, S2, A1, A2> BitOr<&IndexSet<T, S2, A2>> for &IndexSet<T, S1, A1>
where
    T: Eq + Hash + Clone,
    S1: BuildHasher + Default,
    S2: BuildHasher,
...
impl<T, S1, S2, A1, A2> BitXor<&IndexSet<T, S2, A2>> for &IndexSet<T, S1, A1>
where
    T: Eq + Hash + Clone,
    S1: BuildHasher + Default,
    S2: BuildHasher,
...
impl<T, S1, S2, A1, A2> Sub<&IndexSet<T, S2, A2>> for &IndexSet<T, S1, A1>
where
    T: Eq + Hash + Clone,
    S1: BuildHasher + Default,
    S2: BuildHasher,
